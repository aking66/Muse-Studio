...
function parseSceneBlock(
    ...
    const rawNum =
        extractField(block, "SCENE_NUM") || extractField(block, "NUMBER");
...
async function* generateOpenAICompatText(opts: {
    ...
    if (!opts.apiKey)
        throw new Error(`${opts.providerName ?? "API"} key not configured.`);
    ...
    try {
        res = await fetch(`${opts.baseUrl}/chat/completions`, {
            ...
            headers: {
                ...
                ...(opts.baseUrl.includes("anthropic") && {
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // #region agent log
        fetch(
            "http://127.0.0.1:7792/ingest/28803232-41f8-4ca2-8286-1055ebb53327",
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                ...
                body: JSON.stringify({
                    ...
                    hypothesisId: "H2",
                    location:
                        "app/api/generate/scenes/route.ts:POST:parseError",
        ...
        return new Response(
            JSON.stringify({
                error: "Invalid JSON body for scenes generation",
            }),
            {
                status: 400,
            },
    ...
    if (!projectRow?.storyline_plot_outline) {
        return new Response(
            JSON.stringify({ error: "Project has no confirmed storyline" }),
            { status: 400 },
        );
    }
    ...
    const charactersRaw: string[] = projectRow.storyline_characters
        ? JSON.parse(projectRow.storyline_characters)
        : [];
    const themes: string[] = projectRow.storyline_themes
        ? JSON.parse(projectRow.storyline_themes)
        : [];
    ...
    const openaiMaxOutput = Math.min(
        16_384,
        Math.max(4_096, Math.round(600 * targetScenes)),
    );
    const openaiStreamTimeoutMs = Math.min(
        60 * 60 * 1000,
        120_000 + targetScenes * 25_000,
    );
    const claudeMaxOutput = Math.min(
        8_192,
        Math.max(4_096, Math.round(520 * targetScenes)),
    );

    const sceneSystemPrompt = buildSceneSystemPrompt(targetScenes);

    // Scale Ollama token limit and timeout for large scene counts
    const ollamaNumPredict = Math.min(
        32_000,
        Math.max(3_000, 500 * targetScenes),
    );
    const ollamaTimeoutMs =
        15 * 60 * 1000 + Math.max(0, targetScenes - 10) * 60 * 1000;
            ...
            const importFields = [
                {
                    ...
                    label: "Logline",
                    value: loglineRaw
                        ? loglineRaw.slice(0, 80) +
                          (loglineRaw.length > 80 ? "…" : "")
                        : "(none)",
                },
                {
                    field: "plotOutline",
                    label: "Plot Outline",
                    value:
                        plotOutlineRaw.slice(0, 60) +
                        (plotOutlineRaw.length > 60 ? "…" : ""),
                },
                {
                    field: "characters",
                    label: "Characters",
                ...
                },
                {
                    field: "themes",
                    label: "Themes",
                    value: `${themes.length} theme${themes.length !== 1 ? "s" : ""}`,
                },
                {
                    field: "genre",
                    label: "Genre",
                    value: genre || "Unspecified",
                },
            ];

            for (let i = 0; i < importFields.length; i++) {
                enqueue(
                    sseNamedEvent("import", {
                        ...importFields[i],
                        index: i,
                        total: importFields.length,
                    }),
                );
            }

            enqueue(
                sseNamedEvent("generating", {
                    message: "Story Muse is writing your scene scripts…",
                }),
            );
            ...
            try {
                for await (const chunk of generator) {
                    ...
                    while (true) {
                        ...
                        if (endIdx === -1) break;
                        const startIdx = accumulated.lastIndexOf(
                            "<<<SCENE>>>",
                            endIdx,
                        );
                        if (startIdx === -1) break;

                        const block = accumulated.substring(
                            startIdx + 11,
                            endIdx,
                        );
                        ...
                        if (!scene) {
                            console.warn(
                                "[scenes] Unable to parse scene block, skipping.",
                            );
                            continue;
                        }
                ...
                if (!streamClosed)
                    safeEnqueue(
                        sseNamedEvent("error", { message: String(err) }),
                    );
            } finally {
                streamClosed = true;
                clearInterval(keepAlive);
                ...
                while (true) {
                    ...
                    if (!scene) {
                        console.warn(
                            "[scenes] Unable to parse trailing scene block, skipping.",
                        );
                        continue;
                    }
                    ...
                    try {
                        ...
                        console.error(
                            "[scenes] DB insert error (final pass):",
                            dbErr,
                        );
                    }
                }
                ...
                const underfilled =
                    parsedCount > 0 && parsedCount < targetScenes;
                try {
                    if (!streamClosed)
                        controller.enqueue(
                            sseNamedEvent("done", {
                                totalScenes: parsedCount,
                                underfilled,
                            }),
                        );
                    controller.close();

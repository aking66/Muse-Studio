...
import { AlertCircle, Check, RefreshCcw, Trash2, Network, X } from 'lucide-react';
...
function StatusBadge({ healthStatus }: { healthStatus: string }) {
  ...
  const label =
    normalized === 'unhealthy:timeout'
      ? 'unreachable (timeout)'
      : normalized.startsWith('unhealthy:')
        ? `unhealthy (${normalized.split(':')[1]})`
        : healthStatus;
  const className =
...
export default function McpExtensionSettingsPage() {
  ...
  function handleAddExtension() {
    ...
    startTransition(async () => {
      try {
        await installPluginFromLocalUrl({ baseUrl: url });
        setBaseUrl('');
        reload();
      } catch (e) {
  ...
  function handleDelete(pluginId: string) {
    ...
    startTransition(async () => {
      try {
        const ok = window.confirm('Remove this MCP server / extension? This cannot be undone.');
        if (!ok) return;
  ...
  const isMcp = Boolean(details?.manifest?.mcp?.endpointUrl);

  return (
    <div className="space-y-8">
      <div className="space-y-2">
        <h1 className="text-lg font-semibold">MCP Extension</h1>
        <p className="text-sm text-muted-foreground">
          Muse Studio acts as an <strong>MCP client</strong>. Add a <strong>Streamable HTTP</strong> MCP server
          (default path <code className="text-xs">POST /mcp</code>). Example: Z-Image Turbo MCP at{' '}
          <code className="text-xs">http://127.0.0.1:18182</code> — you can paste the base URL;{' '}
          <code className="text-xs">/mcp</code> is appended automatically when needed.
        </p>
        <p className="text-xs text-muted-foreground/70">
          Optional: legacy Muse HTTP plugins that serve <span className="font-mono">/plugin.manifest.json</span> are
          still detected first; if no manifest is found, the URL is tried as MCP.
        </p>
      </div>

      <section className="rounded-2xl border border-white/8 bg-white/3 p-5 space-y-4">
        <h2 className="text-sm font-semibold">Connect MCP server</h2>
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            placeholder="http://192.168.56.1:18182/mcp"
            value={baseUrl}
            onChange={(e) => setBaseUrl(e.target.value)}
          />
          <p className="text-xs text-muted-foreground/60">
            After connect, Muse runs <span className="font-mono">tools/list</span> and registers each tool (for
            example <span className="font-mono">zimage_generate</span> maps to capability{' '}
            <span className="font-mono">image.generate</span>).
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Button onClick={handleAddExtension} disabled={!baseUrl.trim() || isPending}>
            <Network className="h-3.5 w-3.5 mr-1.5" />
            Add server
          </Button>
          <Button variant="outline" onClick={() => setBaseUrl('')} disabled={!baseUrl.trim() || isPending}>
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold">Configured servers ({plugins.length})</h2>
          <Badge>MCP + legacy HTTP</Badge>
        </div>

        {plugins.length === 0 ? (
          <div className="flex items-center justify-center rounded-2xl border-2 border-dashed border-white/6 py-12">
            <div className="text-center">
              <Network className="h-8 w-8 text-muted-foreground/20 mx-auto mb-3" />
              <p className="text-sm text-muted-foreground/50">No MCP servers registered</p>
              <p className="text-xs text-muted-foreground/30 mt-1">Add a server URL above</p>
            </div>
          </div>
        ) : (
              ...
              <div key={p.id} className="rounded-xl border border-white/8 bg-white/3 px-4 py-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground/60 break-all">
                      Source: <span className="font-mono">{p.sourceUrl}</span>
                    </div>
                ...
                {detailsPluginId === p.id && detailsLoading ? (
                  <div className="mt-3 text-xs text-muted-foreground/60">Loading manifest…</div>
                ) : null}
                  ...
                  <div className="mt-3 rounded-lg border border-white/8 bg-black/10 p-3 space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5 min-w-0">
                        ...
                        {details.manifest.mcp?.endpointUrl ? (
                          <div className="text-[11px] text-muted-foreground/50 font-mono truncate">
                            MCP endpoint: {details.manifest.mcp.endpointUrl}
                          </div>
                        ) : null}
                      </div>
                    ...
                    <div className="space-y-2">
                      <div className="text-xs font-semibold">
                        {isMcp ? 'MCP tools (hooks)' : 'Hooks'} ({details.hooks.length})
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {details.hooks.length === 0 ? (
                          <span className="text-xs text-muted-foreground/60">No hooks declared.</span>
                        ) : null}
                        {details.hooks.slice(0, 8).map((h) => (
                          <Badge key={`${h.capability}:${h.method}:${h.path}`}>
                            {h.capability} · {h.method === 'MCP' ? `MCP:${h.path}` : `${h.method} ${h.path}`}
                          </Badge>
                        ))}
                      </div>
                      {details.hooks.length > 8 ? (
                        <div className="text-xs text-muted-foreground/50">…and {details.hooks.length - 8} more</div>
                      ) : null}
                    </div>

                    <div className="space-y-2">
                      <div className="text-xs font-semibold">UI Extensions (preview)</div>
